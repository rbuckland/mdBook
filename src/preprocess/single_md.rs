...
/// A preprocessor for converting the entire book into a single MD
pub struct SingleMDPreprocessor;

impl SingleMDPreprocessor {
    pub(crate) const NAME: &'static str = "index";

    /// Create a new `SingleMDPreprocessor`.
    pub fn new() -> Self {
        SingleMDPreprocessor
    }
}

impl Preprocessor for SingleMDPreprocessor {
    fn name(&self) -> &str {
        Self::NAME
    }
